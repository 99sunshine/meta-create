...
-- Mirrors lib/matching.ts scoreUserMatch() in plpgsql
--
-- Weights (must sum to 100):
--   Skill_Complement   × 25  (Spec formula)
--   Vision_Complement  × 25  (role complementarity)
--   Interest_Overlap   × 20  (Jaccard similarity)
--   Track_Match        × 20  (hackathon track)
--   Availability       × 10
--
...
DECLARE
  ...
  w_role    constant numeric := 25;
  w_interest constant numeric := 20;
  w_track   constant numeric := 20;
  w_avail   constant numeric := 10;
BEGIN
  -- Load caller profile
  SELECT
    p.skills, p.role, p.interests, p.availability, p.hackathon_track
  INTO cur_skills, cur_role, cur_interests, cur_availability, cur_track
  FROM profiles p
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Track match (exact)
      IF cur_track IS NOT NULL AND cur.p_track IS NOT NULL
         AND lower(cur_track) = lower(cur.p_track) THEN
        t_score := w_track;
      END IF;
      ...
      total_raw := s_score + r_score + i_score + t_score + a_score;
      final_score := least(round(total_raw)::integer, 100);

      -- Top reason
      CASE
        WHEN s_score >= r_score AND s_score >= i_score AND s_score >= t_score THEN
          reason := 'Complementary skills';
        WHEN r_score >= s_score AND r_score >= i_score AND r_score >= t_score THEN
          reason := 'Complementary roles';
        WHEN t_score > 0 THEN
          reason := 'Same hackathon track';
        WHEN i_score > 0 THEN
